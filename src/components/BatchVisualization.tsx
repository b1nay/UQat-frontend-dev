...
import { HiDocumentDownload, HiChartBar, HiEye, HiEyeOff, HiInformationCircle, HiCheckCircle, HiXCircle, HiUpload } from 'react-icons/hi';
import { FiFileText, FiFilter, FiSearch, FiBarChart2, FiAlertTriangle, FiPlus } from 'react-icons/fi';
...
const BatchVisualization = () => {
      ...
      Papa.parse(csvText, {
        ...
        complete: (results) => {
          ...
          const requiredColumns = [
            ...
            "Justification", "Top_Chunk", "Response_LLM"
          ];
          ...
          if (missingColumns.length > 0) {
            setLocalFileError(`CSV is missing required columns: ${missingColumns.join(", ")}`);
            setLoading(false);
            return;
          }
          ...
          const data = results.data.filter(row => 
            row.Question && Object.keys(row).length > 1
          );
  ...
  const filteredData = fileData.filter(row => {
    const matchesSearch = row.Question?.toLowerCase().includes(searchQuery.toLowerCase()) || 
                         row.Answer?.toLowerCase().includes(searchQuery.toLowerCase());
    const matchesConfidence = parseFloat(row.Confidence || 0) >= confidenceFilter;
    ...
    <div className="max-w-7xl mx-auto p-4 mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {processedFiles.length === 0 && !selectedFile ? (
            <div className="col-span-3 p-6 border border-dashed border-gray-300 rounded-lg text-center">
              ...
              <p className="text-gray-500 mb-2">No processed files found on server</p>
              <button 
                onClick={() => setDirectUploadModalOpen(true)}
            ...
            <>
              ...
              {selectedFile && !processedFiles.includes(selectedFile) && (
                <div 
                  className="p-3 border rounded-lg cursor-pointer flex items-center gap-2 bg-blue-50 border-main"
                >
                  <FiFileText className="text-main" />
                  <div className="flex-1 truncate">{getPrettyFileName(selectedFile)} (Local)</div>
              ...
              {processedFiles.map((filename, index) => (
                <div 
                  ...
                  className={`p-3 border rounded-lg cursor-pointer flex items-center gap-2 hover:bg-gray-50 transition-colors ${selectedFile === filename ? 'border-main bg-blue-50' : 'border-gray-200'}`}
                >
                  <FiFileText className={selectedFile === filename ? "text-main" : "text-gray-400"} />
                  <div className="flex-1 truncate">{getPrettyFileName(filename)}</div>
                  <HiChartBar className={`${selectedFile === filename ? 'text-main' : 'text-gray-400'} hover:text-main`} />
                </div>
              ))}
            </>
          )}
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center mb-4">
              {localCsvFile ? (
                <div className="flex items-center justify-center gap-2">
                  ...
                  <span className="font-medium text-gray-800">{localCsvFile.name}</span>
                  <button
                    type="button"
                ...
                <>
                  ...
                  <p className="text-xs text-gray-400 mb-2">
                    Question, Answer, Confidence, Source, Justification, Top_Chunk, Response_LLM
                  </p>
                  <button 
      ...
      {selectedFile ? (
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-between">
              <div className="flex-1">
                <h3 className="text-lg font-medium mb-2">
                  ...
                  {!processedFiles.includes(selectedFile) && <span className="text-blue-500 ml-2">(Local File)</span>}
                </h3>
                <div className="flex gap-3">
                  {processedFiles.includes(selectedFile) && (
                    <button
                      onClick={() => window.open(`http://localhost:8000/api/download/${selectedFile}`, "_blank")}
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                <button
                  ...
                  className={`flex items-center gap-1 py-2 px-3 border rounded-md ${showAIResponses ? 'bg-blue-50 border-blue-200' : 'bg-gray-50 border-gray-200'}`}
                >
                  {showAIResponses ? <HiEye className="text-main" /> : <HiEyeOff className="text-gray-500" />}
            ...
            <div className="p-4 border-b border-gray-200 bg-gray-50">
              <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                <div className="bg-white p-3 rounded-lg border border-gray-200 shadow-sm">
                  <div className="text-gray-500 text-sm mb-1">Total Questions</div>
                  <div className="text-2xl font-bold">{summaryStats.totalQuestions}</div>
                </div>
                
                <div className="bg-white p-3 rounded-lg border border-gray-200 shadow-sm">
                  <div className="text-gray-500 text-sm mb-1">Avg Confidence</div>
                  <div className="text-2xl font-bold text-blue-600">{(summaryStats.avgConfidence * 100).toFixed(1)}%</div>
                </div>
                
                <div className="bg-white p-3 rounded-lg border border-gray-200 shadow-sm">
                  <div className="text-gray-500 text-sm mb-1">High Confidence</div>
                  <div className="flex items-end gap-2">
                    <span className="text-2xl font-bold text-green-600">{summaryStats.highConfidence}</span>
                    <span className="text-sm text-gray-500 mb-1">({Math.round(summaryStats.highConfidence / summaryStats.totalQuestions * 100)}%)</span>
                  </div>
                </div>
                ...
                <div className="bg-white p-3 rounded-lg border border-gray-200 shadow-sm">
                  ...
                  <div className="flex items-end gap-2">
                    <span className="text-2xl font-bold text-yellow-500">{summaryStats.mediumConfidence}</span>
                    <span className="text-sm text-gray-500 mb-1">({Math.round(summaryStats.mediumConfidence / summaryStats.totalQuestions * 100)}%)</span>
                  </div>
                </div>
                ...
                <div className="bg-white p-3 rounded-lg border border-gray-200 shadow-sm">
                  ...
                  <div className="flex items-end gap-2">
                    <span className="text-2xl font-bold text-red-500">{summaryStats.lowConfidence}</span>
                    <span className="text-sm text-gray-500 mb-1">({Math.round(summaryStats.lowConfidence / summaryStats.totalQuestions * 100)}%)</span>
                  </div>
                </div>
              </div>
            </div>
                  ...
                  {filteredData.length === 0 ? (
                    ...
                    filteredData.map((row, index) => (
                      <React.Fragment key={index}>
                        <tr className={expandedRows[index] ? "bg-blue-50" : "hover:bg-gray-50"}>
                          <td className="px-6 py-4">
                            <div className="text-sm font-medium text-gray-900">{row.Question}</div>
                            <div className="text-sm text-gray-600 mt-1">{row.Answer?.length > 150 ? `${row.Answer.substring(0, 150)}...` : row.Answer}</div>
                          </td>
                          <td className="px-6 py-4">
                            <div className="text-sm text-gray-900 font-medium">{row.Source?.split(":")[0]}</div>
                            <div className="text-sm text-gray-500">{row.Source?.split(":")[1]}</div>
                          </td>
                          <td className="px-6 py-4">
                          ...
                          <td className="px-6 py-4 text-center">
                            <button
                              ...
                              className={`p-1.5 rounded-full transition-colors ${expandedRows[index] ? 'bg-blue-100 text-blue-600' : 'hover:bg-gray-100 text-gray-500'}`}
                            >
                              {expandedRows[index] ? <HiEyeOff className="text-lg" /> : <HiEye className="text-lg" />}
                            </button>
                          </td>
                        </tr>
                        {expandedRows[index] && (
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8 text-center">
          ...
          <h3 className="text-lg font-medium text-gray-800 mb-2">Select a batch file to visualize</h3>
          <p className="text-gray-600 mb-6">
            Choose one of the processed questionnaires above or visualize a local CSV
          </p>
          <button
            onClick={() => setDirectUploadModalOpen(true)}
